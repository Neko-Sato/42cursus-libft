...
/*   Updated: 2025/08/19 18:32:53 by hshimizu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_deque_init(t_deque *deque, size_t elemsize)
{
	...
	chanksize = FT_DEQUE_CHANK_BYTES / elemsize;
	if (chanksize < FT_DEQUE_CHANK_SIZE_MIN)
		chanksize = FT_DEQUE_CHANK_SIZE_MIN;
	chank = malloc(chanksize * elemsize);
	if (!chank)
		return (1);
	map = malloc(FT_DEQUE_DEFAULT_MAP_SIZE * sizeof(void *));
	if (!map)
		return (free(chank), 1);
	deque->_map = map;
	deque->_mapsize = FT_DEQUE_DEFAULT_MAP_SIZE;
