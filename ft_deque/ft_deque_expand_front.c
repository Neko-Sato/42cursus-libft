...
/*   Updated: 2025/11/03 12:49:07 by hshimizu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline int	_reserve_map_front_internal(t_deque *deque, size_t n,
...
static inline int	_reserve_map_front(t_deque *deque, size_t n)
{
	size_t	chank_cnt;
	void	**new_map;
	size_t	new_mapsize;
	...
	if (_reserve_map_front_internal(deque, chank_cnt + n, &new_map,
			&new_mapsize))
		return (1);
...
static inline int	_reserve_chank_front(t_deque *deque, size_t n)
{
	size_t	i;

	if (deque->_start_index < n && _reserve_map_front(deque, n))
		return (1);
	i = n;
	while (0 < i)
	{
...
int	ft_deque_expand_front(t_deque *deque, size_t size)
{
	...
	if (deque->_start_offset < size)
	{
		...
		required_chanks = (required_size + deque->_chanksize - 1)
			/ deque->_chanksize;
		if (_reserve_chank_front(deque, required_chanks))
			return (-1);
